...
sample         = range(51005,99999)          # sample runs (list); the last number is not reduced.
sample_bg      = None                        # single background run
sample_cd      = None                        # Cadmium run for instrument background subtraction
wv_file        = 'WV_49822.txt'              # white vanadium integral file (mandatory)
Ei_list        = [60]                       # incident energies (Ei) - from PyChop
Erange         = [-0.8,0.0025,0.9]           # energy transfer range to output in fractions of Ei
trans          = [0.95, 0.95]                # elastic line transmission factors for each Ei
mask           = '/home/tw1085782/MAPS Calibrations/MAPS_Mask_244.xml'         # hard mask
...
sample_mass   = 2           # mass of the sample
sample_fwt    = 431.919     # formula weight of sample
monovan_mass  = 32.67       # Mass of vanadium sample (ask local contact) (Old=9.7513g)
#========================================================

#==================Local Contact Inputs==================
inst = 'MAPS'
cycle = '2024_4'              # cycle number
rbnum = '2420263'               # RB number
fixei = True                    # True for LET since no monitor 3
powdermap = 'MAPS_rings_244.xml'      # rings mapping file - must be .xml format
...
save_dir = f'/data/analysis/{inst}/CYCLE{cycle.replace("_","")}/RB{rbnum}/nxspe'  # Set to None to avoid reseting
...
datadir2 = f'/data/instrument/{inst}/CYCLE20{cycle_shortform.replace("_","")}/RB{rbnum}/'
datadir3 = f'/data/analysis/{inst}/CYCLE20{cycle_shortform.replace("_","")}/RB{rbnum}/'
print(datadir2)
...
config.appendDataSearchDir(datadir3)
